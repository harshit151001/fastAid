...
import { getItemsFromQuery, isAuthenticated } from '../../Helper/Enpoints/Endpoints';

const Navbar = ({ cities, setCity, city, searchQuery, setSearchQuery, history, location, setSearchedItems }) => {
    ...
    <nav style={{ zIndex: 100 }} className="navbar-dark bg-dark">
      <div className="container-fluid d-md-flex p-2 justify-content-between">
        <div className="d-flex justify-content-between">
          ...
          <Link className="text-decoration-none" to={{ pathname: isAuthenticated() ? '/dashboard' : '/login', state: { seller: 1 } }}>
            <button className="btn btn-outline-success d-md-none">{isAuthenticated() ? 'Dashboard' : 'Log in'}</button>
          </Link>
        </div>
        <div className="d-md-flex">
          <form onSubmit={e => e.preventDefault()} className="d-flex mx-md-2 mt-2 mt-md-0">
          ...
          <form onSubmit={redirectToSearch} className="d-flex mx-md-2 mt-2 mt-md-0">
        ...
        <Link
          to={{ pathname: isAuthenticated() ? '/dashboard' : '/login', state: { seller: 1 } }}
