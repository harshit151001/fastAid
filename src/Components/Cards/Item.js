...
const Item = ({ name, id, companyName, city, contactNumber, stock, address, user, category, likes, dislikes, createdAt, updatedAt }) => {
  ...
  return (
    <div style={{ zIndex: '0' }} className="col-md col-md-6">
      ...
      >
        ...
        <div className="text-muted mb-0 overflow-hidden truncate">{companyName || ''}</div>
        <div className="mb-0 lead">
          <strong>{city.name}</strong>
        </div>
        <div className="mb-0 d-inline-block truncate overflow-hidden">{address?.startsWith('https') ? <a href={address}>{address}</a> : address || ''}</div>
        ...
        <div className="text-muted mb-0 overflow-hidden truncate">Last Updated: {updated.toString().split('2021')[0] + ' 2021'}</div>
        <div className="text-muted mb-0 overflow-hidden truncate">Created: {created.toString().split('2021')[0] + ' 2021'}</div>
      </div>
    </div>
  );
};
